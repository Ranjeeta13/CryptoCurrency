...
class DetailsFragment : Fragment() {
    ...
    private fun loadChart(item: CryptoCurrency) {
        ...
        binding.detaillChartWebView.setLayerType(View.LAYER_TYPE_SOFTWARE,null)
        binding.detaillChartWebView.loadUrl(
            "https://s.tradingview.com/widgetembed/?frameElementId=tradingview_76d87&symbol="+ item.symbol + "USD&interval=D&hidesidetoolbar=1&hidetoptoolbar=1&symboledit=1&saveimage=1&toolbarbg="+
                    "F1F3F6&studies=[]&hideideas=1&theme=Dark&style=1&timezone=Etc%2FUTC&studies_overrides={}&overrides={}&enabled_features"+
                    "[]&disabled_features=[]&locale=en&utm_source=coinmarketcap.com&utm_medium=widget&utm_campaign=chart&utm_term=BTCUSDT"
    ...
    private fun setUpDetails(data: CryptoCurrency) {
        ...
        binding.detailPriceTextView.text =
            String.format(Locale.US, "$%.4f", data.quotes[0].price)
        ...
        binding.detailChangeImageView.setImageResource(
            if (change > 0) R.drawable.ic_caret_up else R.drawable.ic_caret_down
        )
